...
Within this repository, CDUI exclusively means Conversational Driven User Interface.
...
Clarification replaces guessing
...
Traditional website UX

CDUI Runtime architecture

User adapts to a predesigned layout

Interface adapts to the user's current objective

Static pages, fixed routes, and URL paths

Conversational screen states at the generated-interface layer

Navigation bars, headers, footers, and prebuilt layouts

Blank runtime canvas; the requested interface is materialized when needed

Predetermined menus and information hierarchies

Intent-driven component selection and composition

Designed interaction paths

Context-aware interaction that can mutate during the conversation

Invalid input usually produces validation or fallback states

Ambiguous intent can produce a purpose-built clarification interface
...
locate information manually across scattered views;

unravel complex, nested page hierarchies;

learn the vocabulary and information architecture chosen by the application designer; and

follow rigid paths mapped out before their specific objective was known.
...
Selects among predefined screens and states

User follows designed navigation

Config-Driven UI

Stored configuration

Renders a configured arrangement of known components

Configuration changes the arrangement

Server-Driven UI

Server-provided representation

Renders a server-selected or server-composed view

Server changes the delivered structure

Conversational Driven UI

Interpreted conversational intent expressed through a constrained schema

Generates, revises, or clarifies the next interface state
...
Receive intent — The user expresses an objective or interacts with the current interface.

Assemble context — The runtime collects relevant conversation state, portfolio data, supported capabilities, and prior interaction results.

Interpret — The intelligence layer determines whether the request is actionable or ambiguous.

Describe — It returns a constrained ScreenDescription or clarification state.

Validate — The runtime checks the description against its schema and registered capabilities.

Render — Approved widgets and actions are mapped to trusted React components.

Continue — The resulting interaction updates context and starts the next cycle.
...
interpret the visitor's comparison objective;

identify the relevant portfolio data;

generate a comparison-oriented ScreenDescription;

render matching project cards, technology relationships, and relevant actions; and

reshape the interface when the visitor asks to focus on machine learning, industrial integration, or project outcomes.
...
See LICENSE for the complete governing terms when that file is included in the repository.
